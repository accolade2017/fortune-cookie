...
public class SummaryController {
	 ...
	 */
	   ...
	   //取得用SQL
	   private static final String GET_SQL = "select   us.login_id, sum(frs.score) as score from  fortunes frs inner join users_fortunes ufs on frs.fortune_id = ufs.fortune_id inner join users us on ufs.user_id = us.user_id where SUBSTR(ufs.fortune_ymd, 0, 6) =%s group by us.login_id,SUBSTR(ufs.fortune_ymd, 0, 6)" ;
	   //プルダウン用
	   ...
	   public String sample(Model model) {
		   ...
		   String date = DateTimeFormatter.ofPattern("uuuu/MM/dd").format(date1);
